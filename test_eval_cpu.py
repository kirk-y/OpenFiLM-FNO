"""
CPU推理测试脚本 (基于 test_eval.py 修改)
功能: 在CPU上运行模型推理，计算指标并生成图表
移除所有GPU相关代码，强制使用CPU设备
"""
import json
import os
import shutil
...
def main(config):
    ...
    # 强制使用CPU设备（覆盖config中的设置）
    device = torch.device('cpu')
    print(f"强制使用设备: {device} (CPU推理测试)")
    ...
    test_loader = DataLoader(
        ...
        pin_memory=False,  # CPU推理，禁用pin_memory
    ...
    # Initialize Trainer just for helper methods (normalization, etc)
    ...
    # Aggregating per condition stats (移除了GPU相关指标)
    final_cond_metrics = {}
